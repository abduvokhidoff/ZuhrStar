...
export default function Tolovlar() {
	...
	const filtered = useMemo(() => {
		...
		if (fStart)
			arr = arr.filter(c => (c.date_Of_Create || '').slice(0, 10) >= fStart)
		if (fEnd)
			arr = arr.filter(c => (c.date_Of_Create || '').slice(0, 10) <= fEnd)
		if (fCourse) arr = arr.filter(c => (c.course || '') === fCourse)
		if (fGroup) {
			arr = arr.filter(c => {
				if (String(c.group_id || '') === String(fGroup)) return true
				const s = students.find(
					x => String(x.student_id) === String(c.paid_student_id)
				)
		...
		if (q) {
			arr = arr.filter(c => {
				...
				const hay = `${fullname || ''} ${c.course || ''} ${
					c.group_name || ''
				} ${c.method || ''} ${c.payment_id || ''}`
					.toLowerCase()
					.trim()
	...
	const updateStudentPaid = async (student_id, paidBool) => {
		try {
			const current = await fetchWithAuth(
				`${API_BASE}/api/students/${student_id}`,
				{
					method: 'GET',
				}
			)
	...
	const filteredStudents = useMemo(() => {
		...
		return students.filter(s => {
			const fullName = `${s?.surname || ''} ${s?.name || ''}`.toLowerCase()
			const phone = (s?.student_phone || '').toLowerCase()
			const id = String(s?.student_id || '').toLowerCase()
			return (
				fullName.includes(term) || phone.includes(term) || id.includes(term)
			)
		})
	...
	const filteredGroups = useMemo(() => {
		...
		return groups.filter(g => {
			...
			const id = String(g?.group_id || '').toLowerCase()
			return name.includes(term) || course.includes(term) || id.includes(term)
		})
	...
	const openEdit = async payment_id => {
		try {
			const c = await fetchWithAuth(`${API_BASE}/api/checks/${payment_id}`)
			setForm({
				student_id: c.paid_student_id ?? '',
				fullName: '',
				group_id: '',
				group_name: '',
				course: '',
			...
			if (s) {
				const name = `${s?.surname || ''} ${s?.name || ''}`
					.trim()
					.replace(/\s+/g, ' ')
				setStudentSearchTerm(`${name} — ${s.student_phone || ''}`)
				setForm(prev => ({
					...prev,
					fullName: name,
				}))
			...
			if (c.group_id) {
				const g = groups.find(x => String(x.group_id) === String(c.group_id))
				if (g) {
					setGroupSearchTerm(`${g.name} — ${g.course}`)
					setForm(prev => ({
						...prev,
						group_id: c.group_id,
						group_name: g.name,
			...
			console.error(e)
			setError('Chekni yuklashda xatolik.')
		}
	}
	...
	const onSelectStudent = student => {
		const name = `${student?.surname || ''} ${student?.name || ''}`
			.trim()
			.replace(/\s+/g, ' ')
		setStudentSearchTerm(`${name} — ${student.student_phone || ''}`)
		setForm(p => ({
			...p,
	...
	const onSelectGroup = group => {
		setGroupSearchTerm(`${group.name} — ${group.course}`)
		setForm(p => ({
			...p,
			group_id: group.group_id,
			group_name: group.name,
			course: group.course || p.course,
		}))
	...
	const saveCheck = async e => {
		...
		try {
			if (isEditing && selectedPaymentId) {
				await fetchWithAuth(`${API_BASE}/api/checks/${selectedPaymentId}`, {
					method: 'PUT',
					body: JSON.stringify(payload),
				})
			} else {
	...
	const onPickImportFile = async e => {
		...
		try {
			...
			setMapping(m => ({
				...
				fullName:
					guess(['fullname', 'fio', 'f.i.sh', 'name', 'ism', 'familiya']) ||
					m.fullName,
					...
					<div className='fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50'>
						...
						>
							<h2 className='text-xl font-semibold mb-4'>
								{isEditing ? 'Chekni tahrirlash' : "Yangi to'lov qo'shish"}
							</h2>
									...
									<div className='relative'>
										...
										{studentSearchTerm && (
											<button
												...
												onClick={() => {
													setStudentSearchTerm('')
													setForm(p => ({ ...p, student_id: '', fullName: '' }))
												}}
										...
										{showStudentDropdown && (
											<div className='absolute z-[5] w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto'>
														...
														return (
															...
															>
																...
																<div className='text-xs text-gray-500 mt-0.5'>
																	{s.student_phone || '—'} • ID: {s.student_id}
																	{s.paid && (
												...
												<thead className='bg-white'>
													<tr>
														{importCols.map(c => (
															<th
																key={c}
																className='px-3 py-2 text-left border-b text-gray-600'
															>
																{c}
															</th>
														))}
									...
									{debtors.map((c, i) => (
										...
										>
											...
											<td className='px-3 py-2'>
												{c.fullName ||
													c.student_name ||
													getStudentName(c.paid_student_id) ||
													'—'}
											</td>
											...
											<td className='px-3 py-2'>
												<span className='px-2 py-0.5 rounded text-white bg-red-500'>
													qarzdor
												</span>
											</td>
											<td className='px-3 py-2'>
				...
				<div className='bg-white rounded-lg shadow-sm overflow-hidden'>
					...
					<div>
						{loading ? (
							...
							filtered.map((c, idx) => {
								const MethodIcon =
									methods.find(m => m.value === c.method)?.icon || Receipt
