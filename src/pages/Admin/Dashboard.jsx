...
export default function Dashboard() {
	...
	const dailyRevenue = useMemo(() => {
		...
		checks.forEach(c => {
			...
			if (day) day.amount += number(c?.amount ?? c?.summa ?? c?.price ?? c?.total ?? 0)
		})
		return days
	}, [checks])
	...
	const last5Students = useMemo(() => {
		const copy = [...students]
		copy.sort(
			(a, b) => (safeDate(b)?.getTime?.() || 0) - (safeDate(a)?.getTime?.() || 0)
		)
			...
			<div className='mx-auto max-w-7xl p-6'>
				...
				<div className='mb-6 grid grid-cols-1 gap-4 lg:grid-cols-2'>
					<ChartCard
						...
						valueFormatter={i => fmtNum(monthlyRevenue[last12Months[i].label] || 0)}
					/>
					<ChartCard
				...
				<div className='grid grid-cols-1 gap-4 lg:grid-cols-2'>
					<ListCard
						...
						items={last5Students.map(s => {
							const full =
								(s.fullName || `${s?.surname || ''} ${s?.name || ''}`.trim()).trim() ||
								'—'
...
// ===== CHARTCARD =====
function ChartCard({ title, labels, values, stroke, fillFrom, fillTo, valueFormatter }) {
	const data = {
		labels,
		datasets: [
			{
							...
							>
								<div>
									<div className='flex flex-col'>
										<span className='font-medium text-slate-900'>{it.primary}</span>
										<span className='text-sm text-slate-500'>{it.secondary}</span>
									</div>
								</div>
