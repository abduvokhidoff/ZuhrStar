...
export default function Kurslar() {
		...
		methodology: [
			{
				...
				lessons: [
					{ title: 'Введение в HTML', description: 'Основы HTML разметки' },
				],
			},
		],
	})
	...
	const courseIdMatchesGroup = (crs, grp) => {
		...
		const byName =
			(grp?.course &&
				courseName(crs) &&
				String(grp.course) === courseName(crs)) ||
	...
	const openCourseDetail = async course => {
		...
		try {
			...
			const courseGroups = groupsList.filter(gr =>
				courseIdMatchesGroup(course, gr)
			)
			const now = new Date()
	...
	const handleSubmit = async e => {
		...
		try {
			if (isEditing && selectedCourse) {
				...
				setCourses(prev =>
					prev.map(c =>
						c._id === selectedCourse._id ? { ...c, ...sanitizedData } : c
					)
				)
			} else {
				...
				const newCourse = Array.isArray(created)
					? created[0]
	...
	const detailData = useMemo(() => {
		...
		const courseStudents = students.filter(stu =>
			studentBelongsToAnyGroup(stu, groupIdSet)
		)
						...
						>
							...
							<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
								...
								<select
									...
									onChange={e =>
										setFormData(p => ({ ...p, duration_type: e.target.value }))
									}
									required
									className='w-full border border-gray-300 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500'
								>
									...
									detailData.courseGroups.map(g => {
										...
										return (
											<span
												...
												title={
													expired
														? 'Окончание длительности курса — группа просрочена'
														: 'Активная группа'
												}
											>
