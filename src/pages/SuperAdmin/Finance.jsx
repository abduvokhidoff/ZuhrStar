...
const getTeacherName = teacher => {
	...
	return (
		[firstName, lastName].filter(Boolean).join(' ') || "Noma'lum o'qituvchi"
	)
}
...
export default function Finance() {
	...
	useEffect(() => {
		...
		const fetchData = async () => {
			try {
				...
				// Parallel API calls
				const [studentsData, teachersData, salariesData, checksData] =
	...
	const salaryMap = useMemo(() => {
		...
		salaries.forEach(salary => {
			const teacherId =
				salary?.teacherId || salary?.teacher_id || salary?.teacher || salary?.id
	...
	const totals = useMemo(() => {
		const totalSalaries = teacherRows.reduce(
			(sum, row) => sum + n(row.totalSalary),
			0
		)
	...
	const revenueMonthly = useMemo(() => {
		...
		checks.forEach(check => {
			const dateStr =
				check?.date || check?.createdAt || check?.paidAt || check?.updatedAt
					...
					<div className='w-full max-w-3xl space-y-4 sm:space-y-6 px-2'>
						...
						<div className='grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4'>
							{Array.from({ length: 4 }).map((_, i) => (
								<div
									key={i}
									className='h-20 sm:h-24 rounded-xl sm:rounded-2xl bg-white/70 backdrop-blur-sm animate-pulse shadow-lg'
								/>
							))}
						</div>
			...
			<div className='max-w-7xl mx-auto space-y-4 sm:space-y-6 lg:space-y-8'>
				...
				<header className='flex flex-col gap-3 pt-2'>
					<div className='flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3'>
						<div>
							...
							<p className='text-sm sm:text-base text-slate-600/90'>
				...
				<section className='grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 lg:gap-6'>
					...
					<div className='rounded-xl sm:rounded-2xl bg-white/80 ring-1 ring-slate-200 backdrop-blur-sm p-3 sm:p-4 lg:p-5 shadow-lg hover:shadow-xl transition-all'>
						...
						<div className='text-xl sm:text-2xl lg:text-3xl font-black text-slate-800'>
							{studentStats.total}
						</div>
					...
					<div className='rounded-xl sm:rounded-2xl bg-gradient-to-br from-blue-50 to-blue-100 ring-1 ring-blue-200/60 backdrop-blur-sm p-3 sm:p-4 lg:p-5 shadow-lg hover:shadow-xl transition-all'>
						...
						<div className='mt-2 sm:mt-3 h-1 sm:h-1.5 w-full bg-blue-200 rounded-full'>
							<div
								className='h-1 sm:h-1.5 bg-gradient-to-r from-blue-400 to-blue-600 rounded-full'
								style={{
									width: `${
					...
					<div className='rounded-xl sm:rounded-2xl bg-gradient-to-br from-emerald-50 to-emerald-100 ring-1 ring-emerald-200/60 backdrop-blur-sm p-3 sm:p-4 lg:p-5 shadow-lg hover:shadow-xl transition-all'>
						...
						<div className='text-xl sm:text-2xl lg:text-3xl font-black text-emerald-900'>
							{studentStats.paid}
						</div>
						<div className='mt-2 sm:mt-3 h-1 sm:h-1.5 w-full bg-emerald-200 rounded-full'>
							<div
								className='h-1 sm:h-1.5 bg-gradient-to-r from-emerald-400 to-emerald-600 rounded-full'
								style={{
									width: `${
										studentStats.total > 0
							...
							{salaryChartData.length === 0 ? (
								...
								<ResponsiveContainer width='100%' height='100%'>
									<BarChart data={salaryChartData}>
										...
										<Legend
											formatter={v =>
												v === 'salaryM' ? 'Oylik (mln)' : "O'quvchilar"
											}
										/>
										<Bar
											yAxisId='left'
				...
				<section className='rounded-xl sm:rounded-2xl bg-white/80 ring-1 ring-slate-200 backdrop-blur-sm p-4 sm:p-6 shadow-xl hover:shadow-2xl transition-all'>
					...
					<div className='block sm:hidden space-y-3'>
						{teacherRows.length > 0 ? (
							teacherRows.map((row, idx) => (
								<div
									key={row.id || idx}
									className='rounded-lg bg-white/60 backdrop-blur-sm ring-1 ring-slate-200 p-4'
								>
							...
							<table className='min-w-full text-sm'>
								<thead className='sticky top-0 z-10 bg-slate-50/95 backdrop-blur border-b border-slate-200'>
									<tr className='text-left text-[12px] font-semibold uppercase tracking-wider text-slate-600'>
										...
										<th className='px-3 lg:px-5 py-3 hidden md:table-cell'>
											Telefon
										</th>
										<th className='px-3 lg:px-5 py-3 text-right'>
											O'quvchilar
										</th>
										<th className='px-3 lg:px-5 py-3 text-right'>
											To'lov qilgan
										</th>
										<th className='px-3 lg:px-5 py-3 text-right hidden lg:table-cell'>
											Bazaviy
										</th>
										<th className='px-3 lg:px-5 py-3 text-right hidden lg:table-cell'>
											Bonus
								...
								<tbody className='divide-y divide-slate-200/60'>
									{teacherRows.length > 0 ? (
										teacherRows.map((row, idx) => (
											<tr
												key={row.id || idx}
												className='odd:bg-white/40 hover:bg-white/70 transition-colors'
											>
												<td className='px-3 lg:px-5 py-3 text-slate-600 font-medium'>
													{idx + 1}
												</td>
												...
												<td className='px-3 lg:px-5 py-3'>
													<span
														className={`inline-flex items-center rounded-full px-2 sm:px-2.5 py-0.5 sm:py-1 text-[10px] sm:text-[11px] font-medium ${
															row.role === 'HeadMentor' ||
															row.role === 'head_mentor'
																? 'bg-purple-100 text-purple-700'
																: row.role === 'Mentor' || row.role === 'mentor'
												...
												<td className='px-3 lg:px-5 py-3 text-right font-semibold text-slate-700'>
													{row.studentCount}
												</td>
												...
												<td className='px-3 lg:px-5 py-3'>
													<span
														className={`inline-flex items-center rounded-full px-2 sm:px-2.5 py-0.5 sm:py-1 text-[10px] sm:text-[11px] font-semibold ${
															row.source === 'API'
																? 'bg-emerald-100 text-emerald-800'
																: 'bg-amber-100 text-amber-800'
														}`}
					...
					<div className='mt-4 sm:mt-6 p-3 sm:p-4 rounded-lg sm:rounded-xl bg-gradient-to-r from-blue-50/80 to-indigo-50/60 ring-1 ring-slate-200'>
						<p className='text-xs sm:text-sm text-slate-600 leading-relaxed'>
							<span className='font-semibold text-slate-700'>💡 Eslatma:</span>{' '}
							Bazaviy oylik — rollar va o'quvchilar soni asosida avtomatik
							hisoblanadi. Agar API'dan oylik ma'lumoti kelsa, u ustuvor olinadi
							va "API" sifatida belgilanadi. Hisoblangan oyliklar esa
							"Hisoblangan" deb ko'rsatiladi.
						</p>
					</div>
				</section>
