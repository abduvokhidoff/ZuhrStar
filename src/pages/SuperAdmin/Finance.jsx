        ...
        <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6 lg:space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="rounded-lg sm:rounded-xl bg-white border border-gray-200 p-3 sm:p-4 lg:p-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            {Array.from({ length: 2 }).map((_, i) => (
              <div key={i} className="rounded-lg sm:rounded-xl bg-white border border-gray-200 p-4 sm:p-6">
        ...
        <div className="min-h-[60vh] grid place-items-center px-2">
          <div className="flex flex-col sm:flex-row items-center gap-4 rounded-lg p-4 sm:p-6 bg-white shadow-lg max-w-lg border border-gray-200">
            ...
            <div className="text-center sm:text-left">
              <h3 className="text-lg font-semibold text-gray-800">Xatolik yuz berdi</h3>
      ...
      <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6 lg:space-y-8">
        ...
        <header className="flex flex-col gap-3 pt-2">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
            <div>
              ...
              <p className="text-sm sm:text-base text-gray-600">O'quvchilar to'lovi, o'qituvchilar oyligi va tushumlar tahlili</p>
            </div>
            {updatedAt && (
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
          <div className="rounded-lg sm:rounded-xl bg-white border border-gray-200 p-3 sm:p-4 lg:p-5 shadow-sm hover:shadow-md transition-all">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs sm:text-sm text-gray-600">Jami o'quvchilar</span>
              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400" viewBox="0 0 24 24" fill="currentColor"><path d="M16 11c1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3 1.34 3 3 3Zm-8 0c1.66 0 3-1.34 3-3S9.66 5 8 5 5 6.34 5 8s1.34 3 3 3Zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5C15 14.17 10.33 13 8 13Zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.95 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5Z" /></svg>
            </div>
            <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">{studentStats.total}</div>
          ...
          <div className="rounded-lg sm:rounded-xl bg-white border border-blue-200 p-3 sm:p-4 lg:p-5 shadow-sm hover:shadow-md transition-all">
            ...
            <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-blue-900">{studentStats.active}</div>
            <div className="mt-2 sm:mt-3 h-1 sm:h-1.5 w-full bg-blue-200 rounded-full">
              <div className="h-1 sm:h-1.5 bg-blue-600 rounded-full" style={{ width: `${studentStats.total > 0 ? (studentStats.active / studentStats.total) * 100 : 0}%` }} />
            </div>
          </div>
          ...
          <div className="rounded-lg sm:rounded-xl bg-white border border-green-200 p-3 sm:p-4 lg:p-5 shadow-sm hover:shadow-md transition-all">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs sm:text-sm text-green-700">To'laganlar</span>
              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-green-600" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.2 4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4z" /></svg>
            </div>
            <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-green-900">{studentStats.paid}</div>
            <div className="mt-2 sm:mt-3 h-1 sm:h-1.5 w-full bg-green-200 rounded-full">
              <div className="h-1 sm:h-1.5 bg-green-600 rounded-full" style={{ width: `${studentStats.total > 0 ? (studentStats.paid / studentStats.total) * 100 : 0}%` }} />
            </div>
          </div>
        ...
        <section className="rounded-lg sm:rounded-xl bg-white border border-gray-200 p-4 sm:p-6 shadow-sm hover:shadow-md transition-all">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <h3 className="text-base sm:text-lg font-bold text-gray-800">O'qituvchilar oyligi</h3>
            ...
            {salaryLineData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={salaryLineData}>
                  ...
                  />
                  <YAxis stroke="#6b7280" tick={{ fontSize: window.innerWidth < 640 ? 9 : 12 }} />
                  <RTooltip
                    formatter={(value, name, props) => [
                      money(value),
                      `Oylik (${props.payload.date})`
                    ]}
                    contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.98)', border: '1px solid #e5e7eb', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
                  />
                  <Line
                    type="monotone"
                    dataKey="salary"
                    ...
                    dot={{ fill: '#3b82f6', strokeWidth: 2, r: window.innerWidth < 640 ? 3 : 5 }}
                    activeDot={{ r: window.innerWidth < 640 ? 5 : 7, stroke: '#3b82f6', strokeWidth: 2 }}
                  />
                </LineChart>
              </ResponsiveContainer>
            )}
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          <div className="rounded-lg sm:rounded-xl bg-white border border-gray-200 p-4 sm:p-6 shadow-sm hover:shadow-md transition-all">
            <h3 className="text-base sm:text-lg font-bold text-gray-800 mb-3 sm:mb-4">O'quvchilar holati</h3>
            <div className="h-64 sm:h-80">
              {statusChartData.reduce((s, d) => s + d.count, 0) === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={statusChartData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                    <XAxis dataKey="name" stroke="#6b7280" tick={{ fontSize: window.innerWidth < 640 ? 10 : 12 }} />
                    <YAxis stroke="#6b7280" tick={{ fontSize: window.innerWidth < 640 ? 10 : 12 }} />
                    <RTooltip contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.98)', border: '1px solid #e5e7eb', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} />
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      label={({ name, value }) =>
                        window.innerWidth < 640 ? `${value}` : `${name}: ${value}`
                      }
                      labelLine={false}
                    >
        ...
        <section className="rounded-lg sm:rounded-xl bg-white border border-gray-200 p-4 sm:p-6 shadow-sm hover:shadow-md transition-all">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <h3 className="text-base sm:text-lg font-bold text-gray-800">Oylik tushum (checks)</h3>
            ...
            {revenueMonthly.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={revenueMonthly}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                  <XAxis dataKey="month" stroke="#6b7280" tick={{ fontSize: window.innerWidth < 640 ? 9 : 12 }} />
                  <YAxis stroke="#6b7280" tick={{ fontSize: window.innerWidth < 640 ? 9 : 12 }} />
                  <RTooltip
          ...
          {/* Checks Table */}
          <div className="mt-6">
            <h4 className="text-sm sm:text-base font-semibold text-gray-700 mb-3">Barcha to'lovlar</h4>
                    ...
                    <div key={check._id || idx} className="rounded-lg bg-gray-50 border border-gray-200 p-3">
                      ...
                      <div className="space-y-1 text-xs">
                        <div className="flex justify-between">
                          ...
                          <span className="font-mono text-xs text-gray-600">{check.paid_student_id || 'N/A'}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">To'lov ID:</span>
                          <span className="font-mono text-xs text-gray-600">{check.payment_id || '—'}</span>
                        </div>
                        <div className="flex justify-between">
                ...
                <tbody className="divide-y divide-gray-200">
                  {checks.length > 0 ? (
                    checks.map((check, idx) => {
                      ...
                      return (
                        <tr key={check._id || idx} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${check.status === 'paid' || check.status === 'success'
                              ? 'bg-green-100 text-green-800'
                              : 'bg-yellow-100 text-yellow-800'
                              }`}>
                              {check.status || 'pending'}
        ...
        <section className="rounded-lg sm:rounded-xl bg-white border border-gray-200 p-4 sm:p-6 shadow-sm hover:shadow-md transition-all">
          <div className="flex flex-col gap-4 mb-4 sm:mb-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <h3 className="text-base sm:text-lg font-bold text-gray-800">O'qituvchilar batafsil ma'lumot</h3>
              <button
          ...
          <div className="block sm:hidden space-y-3">
            {salaries.length > 0 ? (
              salaries.map((salary, idx) => (
                <div key={salary._id || idx} className="rounded-lg bg-gray-50 border border-gray-200 p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h4 className="font-semibold text-gray-800 text-sm">{salary?.teacher?.fullName || "Noma'lum"}</h4>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-700">{money(salary?.baseSalary || 0).substring(0, 8)}...</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Bonus:</span>
                      <p className="font-medium text-green-700">{money(salary?.bonuses || 0).substring(0, 6)}...</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Chegirma:</span>
                      <p className="font-medium text-red-700">{money(salary?.deductions || 0).substring(0, 6)}...</p>
                    </div>
                    ...
                    {salary?.notes && (
                      <div className="col-span-2">
                        ...
                        <p className="font-medium text-gray-700 text-xs">{salary.notes}</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr className="text-left text-xs font-semibold uppercase tracking-wider text-gray-600">
                  ...
                  <th className="px-3 lg:px-5 py-3 hidden xl:table-cell">Izoh</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {salaries.length > 0 ? (
                  salaries.map((salary, idx) => (
                    <tr key={salary._id || idx} className="hover:bg-gray-50 transition-colors">
                      <td className="px-3 lg:px-5 py-3 text-gray-600 font-medium">{idx + 1}</td>
                      <td className="px-3 lg:px-5 py-3 font-semibold text-gray-800">
                        <div className="truncate max-w-[150px]">{salary?.teacher?.fullName || "Noma'lum"}</div>
                      </td>
                      <td className="px-3 lg:px-5 py-3">
                        <span className={`inline-flex items-center rounded-full px-2 sm:px-2.5 py-0.5 sm:py-1 text-xs font-medium ${salary?.teacher?.role === 'HeadMentor' || salary?.teacher?.role === 'head_mentor'
                          ? 'bg-purple-100 text-purple-700'
                          : salary?.teacher?.role === 'Mentor' || salary?.teacher?.role === 'mentor'
                            ? 'bg-blue-100 text-blue-700'
                            : 'bg-gray-100 text-gray-700'
                      ...
                      <td className="px-3 lg:px-5 py-3 text-right font-medium text-gray-700">
                        <div className="truncate">{money(salary?.baseSalary || 0)}</div>
                      </td>
                      ...
                      <td className="px-3 lg:px-5 py-3 text-right font-bold text-base text-gray-900">
                        <div className="truncate">{money(salary?.finalSalary || 0)}</div>
                      </td>
                      <td className="px-3 lg:px-5 py-3">
                        <span className={`inline-flex items-center rounded-full px-2 sm:px-2.5 py-0.5 sm:py-1 text-xs font-semibold ${salary?.status === 'paid'
                          ? 'bg-green-100 text-green-800'
                          : salary?.status === 'pending'
                            ? 'bg-yellow-100 text-yellow-800'
                            : 'bg-gray-100 text-gray-800'
