...
import { Plus, X, User, Phone, Mail, Instagram, Calendar, MoreVertical, Trash2, Search, Filter, TrendingUp, Clock } from 'lucide-react';
...
const SOURCE_IMAGES = {
  ...
  friend: 'https://cdn3.iconfinder.com/data/icons/people-pictogram/100/people_friends-512.png',
  default: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/67/OOjs_UI_icon_tag.svg/120px-OOjs_UI_icon_tag.svg.png',
};
...
const normalizeSource = (raw) => {
  ...
  if (s.includes('site') || s.includes('web') || s.includes('landing') || s.includes('website')) return 'website';
  if (s.includes('friend') || s.includes('refer') || s.includes('dost')) return 'friend';
  return 'default';
};
...
const Marketing = () => {
  ...
  const fetchLeads = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://zuhrstar-production.up.railway.app/api/leads/all', {
        method: 'GET',
        headers: {
  ...
  const createLead = async (e) => {
    ...
    try {
      setCreating(true);
      const response = await fetch('https://zuhrstar-production.up.railway.app/api/leads/intake', {
        method: 'POST',
        headers: {
          'accept': 'application/json',
          'Authorization': `Bearer ${accessToken}`,
  ...
  const deleteLead = async () => {
    try {
      ...
      const response = await fetch(`https://zuhrstar-production.up.railway.app/api/leads/${leadToDelete._id || leadToDelete.id}`, {
        method: 'DELETE',
        headers: {
          'accept': 'application/json',
          'Authorization': `Bearer ${accessToken}`
      ...
      if (response.ok) {
        setLeads(prev => prev.filter(lead => (lead._id || lead.id) !== (leadToDelete._id || leadToDelete.id)));
        setTotal(prev => prev - 1);
        ...
        <div className="mb-8">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-6">
            <div>
              ...
              <p className="text-gray-600">Управляйте своими лидами эффективно</p>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              {loading ? (
                ...
                <p className="text-3xl font-bold text-gray-900">{total || filteredLeads.length}</p>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              {loading ? (
                ...
                <p className="text-3xl font-bold text-gray-900">
                  {filteredLeads.filter(lead => normalizeSource(lead.source) === 'instagram').length}
                </p>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredLeads.map((lead, index) => (
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-gray-900 truncate text-lg" title={lead.name}>
                        {lead.name}
                      </h3>
                      <p className="text-sm text-gray-500 truncate" title={lead.username}>
                        @{lead.username}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="relative">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setOpenMenuId(openMenuId === (lead._id || lead.id) ? null : (lead._id || lead.id));
                      }}
                ...
                {/* Status Badge */}
                <div className="mb-4">
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold text-white ${getStatusColor(lead.status)}`}>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  ...
                  {lead.lead_id && (
                    <div className="text-xs font-mono text-gray-400 truncate max-w-[100px]" title={lead.lead_id}>
                      {lead.lead_id}
                    </div>
                  )}
                </div>
