...
function Employees() {
  ...
  const [formData, setFormData] = useState({
    ...
    date_of_birth: '', gender: '', role: 'Mentor',
  })
  ...
  const combinedPeople = [
    ...
    ...(Array.isArray(teachers) ? teachers.map(t => ({ ...t, __kind: 'teacher' })) : []),
  ]

  const formatRole = (role) => {
    switch (role) {
      case 'superadmin': return 'Super Admin'
      case 'HeadMentor': return 'Head Mentor'
      case 'SupportTeacher': return 'Support Teacher'
      case 'admin': return 'Admin'
      default: return role || 'No Role'
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (role === 'admin' || role === 'superadmin') {
      ...
      endpoint = 'https://zuhrstar-production.up.railway.app/api/teachers/register';
      apiData = {
        fullName: formData.fullName,
        phone: formData.phone,
        email: formData.email,
        ...
        skype_username: `${formData.fullName.toLowerCase().replace(/\s+/g, '.')}.skype`,
        imgURL: "https://tse3.mm.bing.net/th/id/OIP.5B1_LC8l5QuZADslXf8HdgHaHa?cb=thvnextc1&rs=1&pid=ImgDetMain&o=7&rm=3"
      };
    }
    ...
    <div className='relative bg-[#F4F9FD] pb-[107px] px-[40px]'>
      <div className='flex justify-between items-center mt-[30px]'>
        <h1 className='max-[360px]:text-[20px] max-[600px]:text-[26px] text-[36px] font-[700] font-[Nunito_sans]'>
          Xodimlar ({(Array.isArray(users) ? users.length : 0) + (Array.isArray(teachers) ? teachers.length : 0)})
        </h1>
        <div className='w-[151px] max-[1000px]:hidden h-[40px] flex justify-center items-center bg-[#3F8CFF] rounded-[20px]'>
          <p className='text-[16px] font-[700] font-[Nunito_sans] text-white'>List</p>
        </div>
        <div className='flex justify-center items-center gap-[24px]'>
      ...
      {(showForm || showEditModal) && (
        <div className="fixed inset-0 z-50 flex justify-center items-center bg-black/40 backdrop-blur-sm">
          <div className="relative h-[600px] bg-white rounded-[16px] p-[24px] w-[90%] max-w-[450px] flex flex-col shadow-lg">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-[12px] h-full">
              <div className="flex flex-col gap-[12px] overflow-y-auto pr-1" style={{ maxHeight: '430px' }}>
                <input name="fullName" value={formData.fullName} onChange={handleInputChange}
                  placeholder="Full Name *" required className='border border-gray-300 rounded-[10px] p-2 outline-none' />
                <input name="email" type="email" value={formData.email} onChange={handleInputChange}
                  placeholder="Email *" required className='border border-gray-300 rounded-[10px] p-2 outline-none' />
                <input name="phone" value={formData.phone} onChange={handleInputChange}
                  placeholder="Phone Number *" required className='border border-gray-300 rounded-[10px] p-2 outline-none' />
                <div className="relative">
                  <input
                ...
                <input name="date_of_birth" type="date" value={formData.date_of_birth} onChange={handleInputChange}
                  className='border border-gray-300 rounded-[10px] p-2 outline-none' />
                <select name="gender" value={formData.gender} onChange={handleInputChange}
                  className='border border-gray-300 rounded-[10px] p-2 outline-none' required={!showEditModal}>
              ...
              <button type="submit" className='bg-[#3F8CFF] text-white py-2 rounded-[10px] font-[700] font-[Nunito_sans] mt-auto'>
      ...
      {showConfirmDelete && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-[10px] w-[300px] text-center shadow">
            <p className="text-lg font-semibold mb-4">Вы точно хотите удалить?</p>
      ...
      <div className="all_employees max-[720px]:justify-center flex justify-between items-center gap-y-[20px] flex-wrap mt-[50px]">
        {pageIsLoading() ? (
          Array.from({ length: 5 }, (_, index) => <EmployeeSkeleton key={index} />)
          ...
          combinedPeople.map(person => (
            <div
              key={person._id || person.id || person.teacher_id || Math.random()}
            ...
            >
              <div className='flex max-[900px]:mb-[20px] max-[900px]:mt-[20px] max-[900px]:flex-col max-[900px]:justify-center max-[900px]:items-center justify-center items-center max-[900px]:gap-[6px] gap-[24px]'>
                <img className='w-[50px]  max-[900px]:w-[100px] max-[900px]:h-[100px] h-[50px]' src={person.imgURL} alt="" />
                <div className='flex flex-col justify-center max-[900px]:items-center items-start max-[900px]:gap-y-[0px] gap-y-[2px]'>
                  ...
                  <p className='text-[#91929E] text-[14px] font-[400] font-[nunito_sans]'>{person.phone || 'No Phone'}</p>
                  <p className='text-[#91929E] text-[12px] font-[400] font-[nunito_sans]'>{person.email || 'No Email'}</p>
                </div>
              </div>
              ...
              <div className='flex max-[380px]:hidden max-[900px]:block max-[900px]:left-[50px] max-[900px]:top-[360px] max-[1000px]:hidden absolute max-[1170px]:left-[320px] left-[388px] flex-col justify-center items-start gap-y-[4px]'>
                <p className='text-[#91929E] text-[14px] font-[400] font-[nunito_sans]'>Gender</p>
                <p className='text-[16px] text-[#0A1629]'>{person.gender || 'No Gender'}</p>
              </div>
              ...
              <div className='flex max-[380px]:hidden max-[900px]:block max-[900px]:left-[160px] max-[900px]:top-[360px] max-[1300px]:hidden absolute left-[536px] flex-col justify-center items-start gap-y-[4px]'>
                <p className='text-[#91929E] text-[14px] font-[400] font-[nunito_sans]'>Date of Birth</p>
                <p className='text-[16px] text-[#0A1629]'>{person.date_of_birth ? person.date_of_birth.slice(0, 10) : 'No Data'}</p>
              </div>
              ...
              <div className='flex max-[380px]:items-center max-[380px]:left-[42%] max-[380px]:top-[320px] max-[1000px]:left-[320px] max-[900px]:left-[50px] max-[900px]:top-[280px] max-[1170px]:left-[390px] max-[1300px]:left-[470px] absolute left-[684px] flex-col justify-center items-start gap-y-[4px]'>
                <p className='text-[#91929E] text-[14px] font-[400] font-[nunito_sans]'>Age</p>
                <p className='text-[16px] max-[900px]:hidden text-[#0A1629]'>{person.date_of_birth ? calculateAge(person.date_of_birth) + ' years old' : 'No Age'}</p>
                <p className='text-[16px] min-[900px]:hidden text-[#0A1629]'>{person.date_of_birth ? calculateAge(person.date_of_birth) + ' y.o' : 'No Age'}</p>
              </div>
              ...
              <div className='flex max-[380px]:top-[250px] max-[380px]:left-[32%] max-[380px]:items-center max-[1000px]:left-[430px] max-[900px]:left-[160px] max-[900px]:top-[280px] z-40 max-[1170px]:left-[500px] max-[1300px]:left-[580px] absolute left-[831px] flex-col justify-center items-start gap-y-[4px]'>
                <p className='text-[#91929E] text-[14px] font-[400] font-[nunito_sans]'>Role</p>
