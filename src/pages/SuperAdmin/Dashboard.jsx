...
const Dashboard = () => {
	...
	useEffect(() => {
		...
		const fetchTeachers = async () => {
			try {
				...
				const teachersData = Array.isArray(res.data?.teachers)
					? res.data.teachers
	...
	const studentStats = useMemo(() => {
		...
		const unpaid = students.filter(
			s => s.paid === false || s.paid === undefined
		).length
				...
				<div className='animate-pulse'>
					<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-6'>
						{[...Array(4)].map((_, i) => (
							<div
								key={i}
								className='bg-white w-full h-[120px] sm:h-[142px] rounded-lg p-4 sm:p-6'
							>
		...
		<div className='min-h-screen p-3 sm:p-4 md:p-6'>
			...
			<div className='grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-6'>
				<div className='bg-white rounded-lg shadow-sm p-4 sm:p-6'>
					<h3 className='text-lg sm:text-[20px] font-[600] text-[#333843] mb-4'>
						Ro'yxatdan o'tish
					</h3>
					<div className='h-48 sm:h-56'>
						{studentRegistrationByMonth.length === 0 ? (
							emptyState
						) : (
							<ResponsiveContainer width='100%' height='100%'>
								<BarChart
									data={studentRegistrationByMonth}
				...
				<div className='flex-1 bg-white rounded-[24px] p-4 sm:p-6'>
					...
					<div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4'>
						{combinedEmployees.map((user, index) => (
							<div
								key={user._id || index}
								className='bg-[#F4F9FD] rounded-[20px] p-3 sm:p-4 flex justify-center text-center items-center flex-col'
							>
								<img
									className='mb-2 w-[40px] h-[40px] sm:w-[45px] sm:h-[45px] rounded-full object-cover'
									src={
										user.imgURL ||
										user.image ||
									...
									alt=''
								/>
								<p className='mb-[2px] text-sm sm:text-[15px] font-[700] truncate w-full'>
